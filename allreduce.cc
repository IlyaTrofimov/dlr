...
void get_kids_vectors(string master_location, char *child_buffer[2], int n, size_t unique_id, size_t total, size_t node, int **child_sockets_out)
{
  ...
  *child_sockets_out = socks.children;
  ...
  while (child_read_pos[0] < n || child_read_pos[1] < n)
  {
	...
	for(int i = 0;i < 2;i++) {
	  if(child_sockets[i] != -1 && FD_ISSET(child_sockets[i],&fds)) {
	    ...
	    addbufs((float*)child_buffer[i] + child_read_pos[i]/sizeof(float), (float*)child_read_buf[i], (child_read_pos[i] + read_size)/sizeof(float) - child_read_pos[i]/sizeof(float));
